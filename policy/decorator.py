...
def policy(obj):
    ...
    if not name.isidentifier():                           # e.g. lambdas -> '<lambda>'
        raise ValueError(f"@policy requires a named def/class, not {name!r}")
    if name in _RESERVED:
        raise ValueError(f"@policy cannot use reserved kernel name {name!r}")
    ...
    if name.startswith(("__", "_repl", "_REPL")):
        raise ValueError(f"@policy: {name!r} uses a kernel-internal prefix "
                         f"(__ / _repl / _REPL); it wouldn't survive crash-restart")
    main_globals = sys.modules["__main__"].__dict__       # ALWAYS the kernel main
    # A policy name shadowing a PREFIX-injected name (a module alias like
    # `_dill`/`_linecache`, or a helper like `policy`/`list_policies`) would break
    # the kernel, which runs in these same globals. `_REPL_INJECTED` is the frozen
    # set of those names. `or ()`: tolerate its pre-freeze None / absence in
    # in-process unit tests (the static `_RESERVED` check above still applies).
    if name in (main_globals.get("_REPL_INJECTED") or ()):
        raise ValueError(f"@policy: {name!r} shadows a kernel-internal name; rename it")
